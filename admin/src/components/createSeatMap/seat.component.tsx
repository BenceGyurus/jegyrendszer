...
const Seat = ({
  ...
  originalColor,
  isSelecting,
}: typeOfSeatParamsType) => {
  const editColorToString = (): string => {
    return `rgb(${editColor.red}, ${editColor.blue}, ${editColor.green})`;
  };
  return (
    <span
      onDoubleClick={(e) => onClickFunction({ ...e, id: seat.id })}
      key={seat?.id}
      className={`editing-seat ${seat?.className}${selectedSeats.includes(seat?.id) ? " shake" : ""} ${isSelecting ? "selecting-seat" : ""}`}
      style={{
        animationDelay: selectedSeats.includes(seat?.id)
          ? `${Math.random()}s`
          : "0s",
        background: !isWatchingGroup
          ? originalColor
            ? seat.color
            : editColorToString()
          : "red",
