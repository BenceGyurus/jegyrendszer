...
const Seat = ({name , title, id, posX, posY, size,newPositionFunction,index, color}:Params)=>{
    return (
        <Draggable onStop={(event:any, data)=>{newPositionFunction(index, data.lastX, data.lastY)}} key = {uuid()}>
            <span className = "seat" style = {{background : color ,width : size, height: size, position: "absolute", top : `${posY}px`, left : `${posX}px`, borderRadius : Math.ceil(size/5)}}>{title}{name ? <span className = "tooltip">{name}</span> : ""}</span>
        </Draggable>
    )
}
