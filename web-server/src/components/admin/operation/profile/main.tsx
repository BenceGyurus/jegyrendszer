...
const EditProfileMain = () => {
  ...
  useEffect(() => {
    postData("/get-user-data", { token: ParseLocalStorage("long_token") }).then(
      (response) => {
  ...
  const changePassword = () => {
    if (pass1 === pass2 && oldPass && pass1.length >= 5) {
      postData("/change-password", {
        token: ParseLocalStorage("long_token"),
        datas: { oldPassword: oldPass, password: pass1 },
      }).then(async (response) => {
  ...
  return (
    <div className="edit-profile-container">
      {edit ? (
        <div className="natification-div">
          <Success
            message={edit}
            closeFunction={() => {
              setEdit("");
