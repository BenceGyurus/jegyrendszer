...
const CreateSeatMapTable = ({
  seats,
  background,
  status,
  widthOfSeats,
  heightOfSeats,
  spaceBetweenTheSeats,
  spaceUnderTheSeats,
  area,
  setArea,
  newSeats,
  ...
  const createSeats = () => {
    newSeats(
      (1 / state.scale) * (area.width / (widthOfSeats + spaceBetweenTheSeats)),
      (1 / state.scale) * (area.height / (heightOfSeats + spaceUnderTheSeats)),
    );
  };
          ...
          for (let k = 0; k < seats[i].seats.length; k++) {
            if (seats[i].seats[k]) {
              for (let j = 0; j < seats[i].seats[k].length; j++) {
                if (
                  seats[i].seats[k][j].x +
                    seats[i].seats[k][j].size.width / 2 >=
                    (selectBox.startX - state.translation.x) *
                      (1 / state.scale) &&
                  seats[i].seats[k][j].x +
                    seats[i].seats[k][j].size.width / 2 <=
                    (selectBox?.endX - state.translation.x) *
                      (1 / state.scale) &&
                  seats[i].seats[k][j].y +
                    seats[i].seats[k][j].size.height / 2 >=
                    (selectBox?.startY - state.translation.y) *
                      (1 / state.scale) &&
                  seats[i].seats[k][j].y +
                    seats[i].seats[k][j].size.height / 2 <=
                    (selectBox?.endY - state.translation.y) * (1 / state.scale)
                ) {
    ...
    if (stages) {
      stages
        .filter((stage) => {
          return (
            stage.x >=
              (selectBox.startX - state.translation.x) * (1 / state.scale) &&
            stage.x <=
              (selectBox?.endX - state.translation.x) * (1 / state.scale) &&
            stage.y >=
              (selectBox?.startY - state.translation.y) * (1 / state.scale) &&
            stage.y <=
              (selectBox?.endY - state.translation.y) * (1 / state.scale)
          );
        })
        .forEach((j) => l.push(j.id));
    }
    setSelectedSeats(l);
  };
  ...
  const generate_Seats = () => {
    ...
    if (isSector) {
      for (
        ...
        i <
        (1 / state.scale) *
          (area.width / (widthOfSeats + spaceBetweenTheSeats));
        i++
      ) {
        for (
          let j = 0;
          j <
          (1 / state.scale) *
            (area.height / (heightOfSeats + spaceUnderTheSeats));
          j++
        ) {
          elements.push(
            <span
              key={uuid()}
              className="editing-seat"
              style={{
                height: heightOfSeats,
                width: widthOfSeats,
                position: "absolute",
                left: i * (widthOfSeats + spaceBetweenTheSeats),
                top: j * (heightOfSeats + spaceUnderTheSeats),
              }}
            ></span>,
          );
        }
      ...
      return (
        <>
          <span
            style={{
              fontSize: 15 * (1 / state.scale),
              top: -20 * (1 / state.scale),
              left: -5 * (1 / state.scale),
            }}
            className="size-of-seat-box-span"
          >
            {Math.floor(
              (1 / state.scale) *
                (area.height / (heightOfSeats + spaceUnderTheSeats)),
  ...
  return (
    <div
      className="create-seat-map-table"
    ...
    >
      <div className="create-map-actions">
        ...
        <button
          ...
          className="create-seat-map-table-restart-button"
        >
      ...
      >
        <div className="table-item-box">
          <Box
            endFunction={selectFunction}
            boxStyle={{ zIndex: 10 }}
            setArea={setSelectionBox}
            scale={state.scale}
            scalePos={state.translation}
            style={{
              width: window.innerWidth * 0.7,
              height: window.innerHeight - 80,
              cursor: "auto",
            }}
            selectingFunction={selectFunction}
            className="selectbox"
            boxClassName="selecting-box"
            width={window.innerWidth * 0.7}
            height={window.innerHeight - 80}
            selecting={status === "edit"}
          >
            <Box
              boxStyle={{ zIndex: 10 }}
              endFunction={createSeats}
              setArea={setArea}
              startFunction={() => {}}
              scale={state.scale}
              scalePos={state.translation}
              ...
              boxChildren={generate_Seats()}
              selecting={status === "create"}
            >
              {status === "drag" ? (
                <Drag
                  style={styleOfDragBox}
                  width={window.innerWidth * 0.7}
                  height={window.innerHeight - 80}
                  startFunction={() => {}}
                  onDragFunction={dragSelectedSeats}
                  endFunction={() => {}}
                />
              ) : (
                <></>
              ...
              <SeatList
                ...
                onClickFunction={setSelectedSeat}
